...
const CustomQuestionAnswerSample = props => {
  ...
  const onCallUpdate = async () => {
    ...
    let selData = questionsData[0].selectedData
      ? questionsData[0].selectedData
      : [];
    let cnt = 0;
    let isChildQuestion = [];
    selData &&
        ...
        switch (item1.question_type) {
          ...
          case '5':
            ...
            let value = {
              ...
              grid_subquestion_id: -1,
              question_answer_id:
                item1.answer && item1.answer.length > 0 ? item1.answer : '',
            };
            allData.push({...value});
                ...
                if (item.selectedObj && item.selectedObj.length > 0) {
                  item.selectedObj.map((i, j) => {
                    if (item.question_answer_id == i.ansId) {
                      ...
                      item1.gird_sub_que_arr.map((item, index) => {
                        if (
                          value.grid_subquestion_id ==
                            item.grid_subquestion_id &&
                          !selectedSubQuestionArr.includes(
                            item.grid_subquestion_id,
                          )
                        ) {
