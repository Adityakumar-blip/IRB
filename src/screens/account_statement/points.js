...
const Points = props => {
  ...
  const [catogoryPointListing, setCatogoryPointListing] = useState([
    ...
    {
      name: I18n.t(globalText.promotion),
      id: 3,
    },
  ]);
  ...
  const onUserPollList = async (currentPage = 0) => {
    ...
    let startDate =
      fromDate && fromDate.toString().length > 0
        ...
        : null;
    let endDate =
      toDate && toDate.toString().length > 0
        ? getFormatedDate(new Date(toDate).toString(), 'yyyy-mm-dd')
        : null;
    ...
    if (
      ...
      startDate = startDate
        ? startDate
        : getFormatedDate(subtractYears(1, new Date(toDate)), 'yyyy-mm-dd');
      endDate = endDate
        ? endDate
        : getFormatedDate(new Date().toString(), 'yyyy-mm-dd');
    }
    const payload = {
    ...
    const isData =
      data && data.data && data.data.final_data ? data.data.final_data : [];
  ...
  const renderItem = ({item, index}) => (
    <View key={index} style={styles.paddB10}>
      <CustomPointHistory
        ...
        converted={
          item.points_requested && item.points_requested
            ? getInThousand(item.points_requested)
            : null
        }
        balance={
          item.remaining_balance ? getInThousand(item.remaining_balance) : null
        }
        ...
        onEarnPopup={() => {
          isSeleValue == 'Earn' + index + 1
            ? setSelValue('')
            : setSelValue('Earn' + index + 1);
        }}
        onConvertPopun={() => {
          isSeleValue == 'Conv' + index + 1
            ? setSelValue('')
            : setSelValue('Conv' + index + 1);
        }}
        onRBalPoppup={() => {
          isSeleValue == 'RemB' + index + 1
            ? setSelValue('')
            : setSelValue('RemB' + index + 1);
        }}
      />
    </View>
  );
      ...
      <View styels={styles.container}>
        ...
        {toShow && (
          <DateTimePickerModal
            ...
            minimumDate={
              fromDate && fromDate.toString().length > 0 && new Date(fromDate)
            }
            onConfirm={date => {
              setToDate(date);
              setToShow(false);
