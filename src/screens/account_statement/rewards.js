...
const Rewards = props => {
    ...
    const [redeemStatus, setRedeemStatus] = useState(true);
    ...
    const [isSelValue, setSelValue] = useState('');
    ...
    const onRewardCategoryListing = async (currentPage = 0) => {
        ...
        let startDate =
            fromDate && fromDate.toString().length > 0
                ? getFormatedDate(new Date(fromDate).toString(), 'mm/dd/yyyy')
                : null;
        let endDate =
            toDate && toDate.toString().length > 0 ? getFormatedDate(new Date(toDate).toString(), 'mm/dd/yyyy') : null;
        ...
        if ((startDate && startDate.length > 0) || (endDate && endDate.length > 0)) {
            startDate = startDate ? startDate : getFormatedDate(subtractYears(1, new Date(toDate)), 'mm/dd/yyyy');
            endDate = endDate ? endDate : getFormatedDate(new Date().toString(), 'mm/dd/yyyy');
        }
        const payload = {
            ...
            dateFrom: startDate,
            dateTo: endDate,
            category: listing_culture_id_value,
        };

        let endPoint = `${Api.rewardTabListing}`;
        ...
        const accData = data && data.data && data.data.data ? data.data.data : [];
        ...
        if (paginationData.currentPage + 1 == Math.ceil(data.data.total_count / 10)) {
            setMore(false);
        } else {
            setMore(true);
        }
    ...
    const renderItem = ({ item, index }) => (
        <View key={index} style={styles.paddB10}>
            <CustomStateHistory
                ...
                description={item.bonus_type}
                earned={item.earn ? getInThousand(item.earn ? item.earn : 0) : 0}
                redemm={item.redemed ? getInThousand(item.redemed ? item.redemed : 0) : 0}
                balance={
                    item.remaining_balance ? getInThousand(item.remaining_balance ? item.remaining_balance : 0) : 0
                }
                // date={getFormatedDate(item.created_on.split(' ')[0], 'dd/mm/yyyy')}
                // date={getFormatedDate(new Date(item.created_onn), 'dd/mm/yyyy')}
                date={item.created_onn}
                openNotification={item.reward_status != 'rejected_reward' && item.isNotification}
                isOpen={isOpen}
                eranColor={
                    item.reward_status == 'approved_reward'
                        ? 'green'
                        : item.reward_status == 'reveiwed_reward'
                        ? 'black'
                        : null
                }
                onPress={() => {
                ...
                message={
                    item.reward_status == 'approved_reward'
                        ? I18n.t(globalText._theRewardIsApprovedForRedemption) + ' (' + item.earn + ')'
                        : item.reward_status == 'reveiwed_reward'
                        ? I18n.t(globalText._yourRewardIsBeingReviewed) + ' (' + item.earn + ')'
                        : item.earn
                }
                redeemColor={item.reward_status == 'rejected_reward' ? 'red' : null}
                onPressRedeem={() => {
                    if (item.reward_status == 'rejected_reward') {
                        // if (isOpen) {
                        //     setOpen(false);
                        //     setRedeemStatus(true);
                        // }
                        item.isRedeem = !item.isRedeem;
                        setAccRewardHistory([...accRewardHistory]);
                    }
                }}
                checkEarn={item.reward_status == 'rejected_reward'}
                checkReddem={item.reward_status != 'rejected_reward'}
                openIsRedeem={item.reward_status == 'rejected_reward' && item.isRedeem}
                isRedeemOpen={redeemStatus}
                onRequestRedeemClose={() => setRedeemStatus(false)}
                redeem_message={
                    item.reward_status == 'rejected_reward'
                        ? I18n.t(globalText._yourRewardHasBeenRejectedOnQuality) + ' (' + item.redemed + ')'
                        : item.redemed
                }
                real_remain_amount={item.remaining_balance}
                zIndex={index}
                isOpenPopOverEarm={isSelValue == 'Earn' + index + 1}
                onPopOverEarn={() => {
                    isSelValue == 'Earn' + index + 1 ? setSelValue('') : setSelValue('Earn' + index + 1);
                }}
                isOpenPopupRedeem={isSelValue == 'Redeem' + index + 1}
                onPopOverRedeem={() => {
                    isSelValue == 'Redeem' + index + 1 ? setSelValue('') : setSelValue('Redeem' + index + 1);
                }}
                isOpenPopupRemB={isSelValue == 'RemB' + index + 1}
                onPopOverRemb={() => {
                    isSelValue == 'RemB' + index + 1 ? setSelValue('') : setSelValue('RemB' + index + 1);
                }}
            />
        </View>
    );
    ...
    const ListHeaderComponent = () => (
        <View style={[styles.row, styles.paddT10, styles.paddB10]}>
            <View style={styles.rewardPointTabFirstView}>
                <Text style={[styles.fontSize12ColorTopazReWardTab, styles.width90]}>
                    {I18n.t(globalText.description)}
                </Text>
            </View>
