...
import { getAsyncStorage, getFormatedDate, toastShow } from '../../utils/customFunctions';
                    ...
                    <SafeAreaView>
                        ...
                        <View style={styles.loginMainScreen}>
                            ...
                            <View style={styles.row}>
                                ...
                                >
                                    <TextInput
                                        ...
                                        style={{ color: '#000' }}
                                        value={
                                            isData && isData.date_of_birth && isData.date_of_birth != ''
                                        ...
                                        onBlur={() => !allValid && simpleValidator.current.showMessageFor('Date')}
                                    />
                                </TouchableOpacity>
                            </View>
                            <View style={styles.row}>
                                ...
                                {simpleValidator.current.message('Date', isData.date_of_birth, 'required') && (
                            ...
                            <View style={styles.completeProfilePersonalGenderView}>
                                ...
                                <TouchableOpacity
                                    // hitSlop={{ top: 50, bottom: 50, left: 50, right: 50 }}
                                    style={[styles.loginPhoneInputView, styles.completeProfilePersonalGenderDropDown]}
                                    onPress={() => setGenderModal(!genderModal)}
                                >
                                    <View style={styles.width90}>
                                        <TextInput
                                            testID="gender"
                                            placeholder={`${I18n.t(globalText.gender)} ${I18n.t(globalText.impSymbol)}`}
                                            value={isData && isData.gender.length > 0 ? isData.gender : ''}
                                            editable={false}
                                            style={[styles.height100, { color: '#000' }]}
                                            //autoFocus={true}
                                            onBlur={!allValid && simpleValidator.current.showMessageFor('gender')}
                                            // onBlur={() =>
                                    ...
                                    <View style={styles.width10per}>
                                        <Entypo name={'triangle-down'} size={25} color={colors.DOVE_GRAY} />
                                    </View>
                                </TouchableOpacity>
                                {genderModal && (
                                    <View style={styles.completeProfilePersonalGenderDropDownOpen}>
                                        <Text
                            ...
                            <View style={styles.row}>
                                <View style={[styles.width10, styles.marL15]} />
                                {simpleValidator.current.message('gender', isData.gender, 'required') && (
                                    <Text style={styles.redColorText}>
                                        {I18n.t(globalText._theMessageMustBeRequired, {
                        ...
                        {show && (
                            <DateTimePickerModal
                                isVisible={show}
                                mode="date"
                                date={isData && isData.date_of_birth ? isData.date_of_birth : new Date()}
