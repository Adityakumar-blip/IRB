...
import { Modal, ScrollView, StatusBar, Text, TouchableOpacity, View, FlatList } from 'react-native';
...
const CountryModal = props => {
    ...
    const renderItem = ({ item, index }) => (
        <TouchableOpacity key={index} onPress={() => onSetValue(item)} style={styles.countryRowview}>
            {item && item.flag ? (
                    ...
                    <ScrollView>
                        <View style={styles.paddB20}>
                            {data && data.length > 0 ? (
                                ...
                                <Text style={styles.textCenter}>{I18n.t(globalText.noRecordFound)}</Text>
                            )}
                        </View>
                    </ScrollView>
                </View>
