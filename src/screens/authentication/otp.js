...
const OTP = props => {
    ...
    const onVerifyNumber = async () => {
        ...
        let endPoint = `${Api.verifyWhetherEmailOrMobile}?username=${loginId.toLowerCase()}`;
    ...
    const onPressLogin = async () => {
        ...
        if (isPreviousValue == loginId) {
            if (Number(randomOtp) == Number(otp)) {
                ...
                setLoader(true);
                const { data, message, success } = await authApi.postDataToServer(Api.login, payload);
                setLoader(false);
                if (!data) {
                    toastShow(I18n.t(globalText.somethingWentWrong));
                    return;
                }
                await setAsyncStorage('Token', JSON.stringify(data && data.data && data.data.token));
                await setAsyncStorage(
                    'Login_Data',
                    JSON.stringify(data && data.data && data.data.data && data.data.data.length > 0),
                );
                toastShow(I18n.t(globalText.loginSuccessfully));
                setLoader(false);
                await setAsyncStorage('Token', JSON.stringify(data && data.data && data.data.token));
                await setAsyncStorage('ProfileImage', JSON.stringify(data && data.data && data.data.imagebuffer));
                await setAsyncStorage('Login_Data', JSON.stringify(data && data.data && data.data.data));
                await setAsyncStorage(
                    'currencySymbol',
                    data && data.data && data.data.data && data.data.data.currency_symbol.toString(),
                );
                await removeAsyncStorage('Random_Otp');
    ...
    return (
        <>
            {loader && <CustomLoader />}
            {isErrorMessage !== '' ? <Text style={styles.errorMsgBoxStyle}>{isErrorMessage}</Text> : null}
            <View style={styles.flexRow}>
                ...
                <View style={styles.loginPhoneInputView}>
                    <TextInput
                        ...
                        value={loginId}
                        onBlur={() => !allValid && simpleValidator.current.showMessageFor('phone')}
                    />
                </View>
            </View>
            ...
            {minutes === 0 && seconds === 0 && (
                ...
                >
                    <Text style={styles.loginForgetPasswordText}>
                        {sendOtp ? I18n.t(globalText.resendOtp) : I18n.t(globalText.sendOtp)}
                    </Text>
                </TouchableOpacity>
            )}
