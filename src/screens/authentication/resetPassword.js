...
const ResetPassword = props => {
    ...
    const onPageInit = async () => {
        let value = (await props.route) && props.route.params && props.route.params.userValue;
    ...
    useEffect(() => {
        if (count != 0) {
            ...
            let valueConfirmPassword = checkPasswordVallidation(isConfirmPasswordValue);
            valueConfirmPassword ? setConfirmPasswordError(false) : setConfirmPasswordError(true);
        }
    ...
    const onResetPassword = async () => {
        ...
        valueNewPassword ? setNewPasswordError(false) : setNewPasswordError(true);
        let valueConfirmPassword = await checkPasswordVallidation(isConfirmPasswordValue);
        valueConfirmPassword ? setConfirmPasswordError(false) : setConfirmPasswordError(true);
        ...
        const { data, message, success } = await AuthApi.postDataToServer(Api.resetPassword, payload);
        setLoader(false);
        if (!data) {
                ...
                <SafeAreaView>
                    ...
                    >
                        <View style={[styles.pad15, styles.height100AlignIJusCenter]}>
                            <Text style={[styles.textCenter, styles.headerTextStyle, styles.paddB40]}>
