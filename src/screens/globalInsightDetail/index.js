...
const GlobalInsightDetail = props => {
  ...
  const onPageInit = async () => {
    setLoader(true);
    const isData =
      (await props.route) && props.route.params && props.route.params.item;
  ...
  const onGlobalInsightTags = async id => {
    const endPoint = `${
      Api.pollSynopsisTags
    }?limit=${10000}&poll_sub_category_type_id=${id}`;
    const {data, message} = await AuthApi.getDataFromServer(endPoint);
    if (!data) {
      toastShow(message);
      return;
                ...
                <ScrollView>
                  <View style={styles.padd15}>
                    ...
                    <TouchableOpacity onPress={() => setModal(true)}>
                      <FastImage
                        style={styles.globInShareIcon}
                        source={GlobalImages.shareIcon}
                      />
                    </TouchableOpacity>
                    <FastImage
