...
const UpdateEmailMobile = (props) => {
  ...
  useEffect(() => {
    Singular.event("UpdateEmailAndMobile");
    onPageInit();
  }, []);
  ...
  const onUpdateEmailOrPhone = async (isValue) => {
    ...
    const { data, message } = await AuthApi.putDataToServer(
      Api.basicInfoUpdateEmailOrPhone,
      payload
    );
    setLoader(false);
    if (!data) {
    ...
    setTimeout(() => {
      toastShow(
        I18n.t(globalText._The) +
          " " +
          msg +
          " " +
          I18n.t(globalText._updatedSucessfully)
      );
    }, 200);
    ...
    await onSetUpdatedEmailOrMobile();
  };
      ...
      <View style={styles.demoSurveyMainModalView}>
        ...
        >
          <View style={styles.editDetailMaimViewStyle}>
            <TouchableOpacity
              style={[styles.alignSelfFlexEnd]}
              onPress={onPressCross}
            >
              <AntDesign name={"close"} size={25} color={colors.ABBEY} />
            </TouchableOpacity>
            {isErrorMessage != "" ? (
              <Text style={[styles.errorMsgBoxStyle, styles.marT10]}>
                {isErrorMessage}
              </Text>
            ) : null}
            ...
            <CustomTextImput
              ...
              placeholder={
                isName == "email"
                  ? I18n.t(globalText.enterEmail)
                  : I18n.t(globalText.enterMobileNo)
              }
              ...
              onBlur={() =>
                !allValid &&
                simpleValidator.current.showMessageFor("phoneOrEmail")
              }
            />
            ...
            {sendOtp && minutes === 0 && seconds === 0 && (
              <TouchableOpacity
                ...
                disabled={seconds != 0}
              >
                <Text style={styles.editDetailTextStyle}>
            ...
            {sendOtp && (
              <View
                style={[styles.passwordVerificationOtpMainView, styles.marT15]}
              >
                <CodeField
                  value={value}
                  caretHidden={false}
                  ...
                  renderCell={({ index, symbol, isFocused }) => (
                    <Text
                      key={index}
                      style={[
                        styles.editDetailCell,
                        isFocused && styles.editDetailFocusCell,
                      ]}
                    >
